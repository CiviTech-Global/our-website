...
/** Absolute path of a stored file. Rejects anything that escapes the root. */
export function resolveStoredPath(storedName: string): string {
  const root = storageRoot();
  const full = path.resolve(root, storedName);
  // Defence in depth: storedName is generated by us and cannot contain a
  // separator, but a future caller passing a name from the database should
  // still not be able to read /etc/passwd.
...
export async function storeFiles(files: IncomingFile[]): Promise<StoredFile[]> {
  ...
  try {
    for (const { file, type } of typed) {
      ...
      await writeFile(path.join(root, storedName), file.buffer, { mode: 0o640 });
      written.push({
