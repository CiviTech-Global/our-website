...
export default function JobsPage() {
  ...
  // Rows by default, unlike the book market: a job has no cover to recognise
  // it by, and the row carries the pay, the place and the arrangement at once
  // — which is what somebody scanning a board is comparing.
  const controls = useListControls({
    ...
    typedFilters: ['category', 'skills', 'salaryMin', 'salaryMax'],
  });
  ...
  const { data, isLoading } = usePublicJobs({
    ...
    salaryMin: digitsOnly(controls.filters.salaryMin) || undefined,
    salaryMax: digitsOnly(controls.filters.salaryMax) || undefined,
  ...
  return (
    <div className="mx-auto w-full max-w-5xl px-4 py-12 sm:px-6 lg:px-8">
      ...
      <ListToolbar
        className="mb-6"
        controls={controls}
        searchPlaceholder={t.market.searchJobs}
        total={data?.total}
        isLoading={isLoading}
        views={['cards', 'table']}
        filters={
          <>
            ...
            <Select
              className="w-40"
              value={controls.filters.workArrangement}
      ...
      {/* The narrower filters sit in their own band: they are typed rather
          than picked, and mixing a free-text box into the row of dropdowns
          made the row read as five equal choices when it is not. */}
      <div className="mb-6 flex flex-col gap-3 rounded-xl border border-surface-200 p-3 sm:flex-row sm:flex-wrap sm:items-center dark:border-surface-300">
        ...
        <div className="flex items-center gap-2">
          ...
          <Input
            className="w-28"
            value={controls.filterInput('salaryMax')}
            inputMode="numeric"
      ...
      {!isLoading && data?.items.length === 0 && (
        <EmptyState
          title={controls.activeCount > 0 ? t.list.noResults : t.market.noJobs}
