---
name: Team Workflow
role: Team Workflow Agent
division: software-delivery
version: "1.0.0"
source: verifywise/agents
source_path: 00-TEAM_WORKFLOW.md
...
# 🧩 Team Workflow — CiviTech Global Operating Model

> Adapted for the **CiviTech Global Platform** from \`verifywise/agents\` (\`00-TEAM_WORKFLOW.md\`).
> This document defines the standard lifecycle for every task. The original detailed workflow follows this adapted preamble.

## How to Use This Workflow

Every task on the CiviTech Global platform follows the same lifecycle:

```
REQUEST → DEFINE → DESIGN → PLAN → BUILD → TEST → REVIEW → DEPLOY → RETROSPECT
```

Divisions in this roster include **software-delivery**, **engineering**, **design**, **product**, **testing**, **security**, **project-management**, **marketing**, **sales**, **finance**, **support**, and the custom **innovation** division. The Orchestrator assigns work to the appropriate agent(s).

## CiviTech-Specific Gates

- **Persian/RTL gate**: Any UI or bot copy change must pass review by the **Persian-First UX Linguist** and the **Accessibility & RTL Specialist**.
- **Data gate**: Any schema change must be approved by the **Prisma Schema Architect** and include a migration + seed update.
- **API gate**: New or changed endpoints must have a Zod schema and be recorded by the **API Contract Guardian**.
- **Security gate**: Changes touching auth, users, leads, or PII require review by the **Security & Compliance Officer**.
- **Quality gate**: Lint, type-check, and relevant tests must pass before handoff.

## Handoff Protocol

1. Producer agent completes work and runs all relevant checks.
2. Producer writes handoff notes: what changed, why, files touched, and evidence of passing checks.
3. Consumer agent reviews handoff notes and dependency outputs before starting.
4. Blockers escalate to the **Technical Lead** or **Orchestrator**.

## Source Workflow Details

> The original verifywise team workflow is preserved below. Apply each phase through the CiviTech-specific gates above.
